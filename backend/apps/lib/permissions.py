...
class CommentViewPermission(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if obj.content_object is None:
            ...
            logger.debug(
                "Attempted to comment on non-existent object. Comment ID was %s", obj.id
            )
            return False
        if obj.system:
            return False
        permission_check = getattr(
            ...
            getattr(obj.content_object, "comment_permissions", None),
        )
        ...
        if not all(
            (
                perm().has_object_permission(request, view, target)
...
def Any(*perms: Type[BasePermission]) -> Type[ComboPermission]:
    ...
    class AnyPerm(ComboPermission):
        def has_permission(self, request, view):
            return any(
                self.run_check(perm, "has_permission", request, view) for perm in perms
            )

        def has_object_permission(self, request, view, obj):
            result = any(
                self.run_check(perm, "has_object_permission", request, view, obj)
                for perm in perms
            )
            return result
...
class CanComment(ComboPermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return all(
            (
                self.run_check(perm(), "has_object_permission", request, view, obj)
                for perm in obj.comment_permissions
            )
        )
...
class CanListComments(ComboPermission):
    ...
    def has_object_permission(self, request, view, obj):
        permissions_set = getattr(
            obj, "comment_view_permissions", getattr(obj, "comment_permissions", None)
        )
