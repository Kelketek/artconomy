...
def replace_db():
    script_path = str(
        Path(settings.BASE_DIR) / 'backend' / 'apps' / 'lib' / 'management' / 'commands' / 'sql' / 'drop.sql'
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args: List, **options: Dict):
        if not (options['force'] or settings.ENV_NAME == 'dev'):
            raise CommandError('You cannot run this in production without the --force flag. It could ruin everything!')
        replace_db()
