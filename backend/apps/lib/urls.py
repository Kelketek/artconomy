...
urlpatterns = [
    ...
    path(
        "v1/comments/<content_type>/<object_id>/<int:comment_id>/",
        views.CommentUpdate.as_view(),
        name="comment",
    ),
    path(
        "v1/comments/lib.Comment/<int:comment_id>/history/",
        views.CommentHistory.as_view(),
        name="comment_history",
    ),
    path("v1/countries/", views.CountryListing.as_view(), name="country_listing"),
