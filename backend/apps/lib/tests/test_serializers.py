...
class TestComments(APITestCase):
    def test_comment_serializer(self):
        ...
        comments = [
            CommentFactory.create(content_object=submission, top=submission)
            for i in range(5)
        ]
        ...
        self.assertEqual(
            data["commenters"],
            sorted([comment.user.username for comment in comments])[:3],
        )
...
class TestNotificationSerializers(EnsurePlansMixin, TestCase):
    def test_new_product(self):
        ...
        product = ProductFactory.create(
            user=user, primary_submission=SubmissionFactory.create()
        )
