...
def create_tags(apps, schema):
    ...
    for tag in Submission.artists.through.objects.all().order_by(
        "submission__created_on"
    ):
        ArtistTag.objects.get_or_create(
            user=tag.user,
            submission=tag.submission,
            display_position=get_next_artist_position(),
        )
    for tag in Submission.characters.through.objects.all().order_by(
        "submission__created_on"
    ):
        CharacterTag.objects.get_or_create(
            character=tag.character,
            submission=tag.submission,
            display_position=get_next_character_position(),
        )
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(create_tags, reverse_code=transpose_tags)]
