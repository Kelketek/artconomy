        ...
        with stripe as stripe_api:
            ...
            for item in transaction_events:
                ...
                if not item['balance_transaction']:
                    continue
                for transaction_record in TransactionRecord.objects.filter(remote_ids__contains=item['id']):
                    ...
                    print(f'Added {item["balance_transaction"]} to {transaction_record.id}')
