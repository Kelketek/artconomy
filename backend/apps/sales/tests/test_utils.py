...
from apps.lib.models import Notification, ORDER_UPDATE, Subscription
...
from apps.sales.constants import IN_PROGRESS, CANCELLED, SUCCESS, UNPROCESSED_EARNINGS, ESCROW, CARD, ACH_MISC_FEES, \
    RESERVE, FAILURE, SHIELD
from apps.sales.models import TransactionRecord, LineItemSim
from apps.sales.tests.factories import InvoiceFactory
...
from apps.sales.utils import claim_order_by_token, \
    available_products, account_balance, POSTED_ONLY, PENDING, destroy_deliverable, get_claim_token, fetch_prefixed, verify_total, \
    default_deliverable, from_remote_id
...
class TransactionCheckMixin:
    def check_transactions(
        ...
        if landscape:
            self.assertEqual(escrow.amount, Money('11.02', 'USD'))
        else:
...
class TestVerifyTotal(EnsurePlansMixin, TestCase):
    ...
    @override_settings(MINIMUM_PRICE=Decimal('100.00'))
...
class TestFromRemoteID(EnsurePlansMixin, TestCase):
    ...
    def test_unsupported_status(self):
        event = base_charge_succeeded_event()['data']
        ...
        self.assertEqual(message, 'Unhandled charge status, pending for remote_ids [\'1234\']')
