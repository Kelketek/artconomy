...
from apps.sales.constants import NEW, PAYMENT_PENDING, IN_PROGRESS, REVIEW, CANCELLED, UNPROCESSED_EARNINGS, \
    ...
    FAILURE, PENDING, CARD, ESCROW, SUCCESS, TERM, PAID
...
from apps.sales.tests.factories import TransactionRecordFactory, DeliverableFactory, \
    StripeAccountFactory, ServicePlanFactory, CreditCardTokenFactory, LineItemFactory
...
class TestWithdrawAll(EnsurePlansMixin, TransactionTestCase):
    ...
    def test_withdraw_amount_matches(self, stripe_transfer):
        ...
        deliverable = DeliverableFactory.create(order__seller=user, status=COMPLETED, payout_sent=False)
        record.targets.add(ref_for_instance(deliverable))
        withdraw_all(user.id)
        transfer = TransactionRecord.objects.get(destination=BANK)
        self.assertCountEqual([target.target for target in transfer.targets.all()], [deliverable, account])
        self.assertEqual(transfer.amount, Money('10.00', 'USD'))
        deliverable.refresh_from_db()
        self.assertEqual(deliverable.payout_sent, True)
        stripe_transfer.assert_called_with(transfer.id, account.id, deliverable.id)
...
class TestStripeTransfer(EnsurePlansMixin, TestCase):
    def test_send_transfer_deliverable(self, mock_stripe):
        ...
        mock_stripe.__enter__.return_value.Transfer.create.return_value = {'id': 'beep', 'destination_payment': 'boop'}
        stripe_transfer(record.id, account.id, deliverable_id=deliverable.id)
    ...
    def test_send_transfer_failed_with_deliverable(self, mock_stripe):
    ...
    def test_send_transfer_deliverable_with_source(self, mock_stripe):
        ...
        source_record.targets.add(ref_for_instance(deliverable))
        account = StripeAccountFactory.create(user=deliverable.order.seller, token='foo')
        mock_stripe.__enter__.return_value.Transfer.create.return_value = {'id': 'beep', 'destination_payment': 'boop'}
        stripe_transfer(record.id, account.id, deliverable_id=deliverable.id)
        record.refresh_from_db()
    ...
    def test_send_transfer_no_deliverable(self, mock_stripe):
        user = UserFactory.create()
        record = TransactionRecordFactory.create(
