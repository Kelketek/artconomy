...
from apps.sales.constants import BASE_PRICE, SHIELD, BONUS, TABLE_SERVICE, TAX, COMPLETED, NEW, QUEUED, CANCELLED, \
    WAITING, PENDING, SUCCESS, PRIORITY_MAP, DELIVERABLE_TRACKING
...
class TestInvoice(EnsurePlansMixin, TestCase):
    def test_string_no_deliverable(self):
        ...
        self.assertEqual(
            ...
            f'Invoice {invoice.id} for {invoice.bill_to.username} in the amount of $\xa00.00',
        )
    ...
    def test_string_deliverable(self):
        ...
        self.assertEqual(
            ...
            f'Invoice {deliverable.invoice.id} for {deliverable.invoice.bill_to.username} in the amount of $15.00 '
            f'for deliverable: Deliverable object ({deliverable.id})',
        )
