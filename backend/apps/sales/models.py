...
class Product(ImageModel, HitsMixin):
    ...
    paypal = BooleanField(
        default=True,
        db_index=True,
        help_text="Whether PayPal invoices are generated by default upon acceptance "
        "for non-shield orders. Ignored if there is not an active PaypalConfig.",
    )
...
class Deliverable(Model):
    ...
    paypal = BooleanField(default=False)
...
class CreditCardToken(Model):
    ...
    created_on = DateTimeField(auto_now_add=True, db_index=True)
    watch_permissions = {"CardSerializer": [Any(UserControls("view_financials"))]}
...
class Invoice(models.Model):
    ...
    watch_permissions = {
        "InvoiceSerializer": [
            Any(UserControls("invoice_financials"), BillTo, IssuedBy)
        ],
        None: [Any(UserControls("view_financials"), BillTo, IssuedBy)],
    }
    ...
    paypal_token = models.CharField(
        default="", db_index=True, max_length=50, blank=True
    )
    currency = CurrencyField(default="USD", blank=True, db_index=True)
    current_intent = CharField(max_length=30, db_index=True, default="", blank=True)
    ...
    def total(self) -> Money:
        return reckon_lines(self.line_items.all(), currency=get_currency(self.currency))
...
class LineItem(Model):
    ...
    paypal_token = CharField(max_length=250, blank=True, default="", db_index=True)
    ...
    watch_permissions = {
        "LineItemSerializer": [Any(OrderViewPermission, BillTo, IssuedBy, StaffPower("table_seller"), StaffPower("view_financials"))]
    }
    targets = ManyToManyField(
...
class Reference(ImageModel):
    ...
    comment_permissions = [Any(ReferenceViewPermission, StaffPower("handle_disputes"), StaffPower("table_seller"))]
    preserve_comments = True
...
class StripeAccount(Model):
    ...
    processor = "stripe"
    watch_permissions = {
        "StripeAccountSerializer": [UserControls("administrate_users")]
    }
...
class PaypalConfig(models.Model):
    """
    Model for holding PayPal API key info for a particular user.
    ...
    template_id = models.CharField(max_length=25, default="", blank=True)
    webhook_id = models.CharField(max_length=25, default="", db_index=True, blank=True)
    active = models.BooleanField(default=False, db_index=True)

    def __str__(self):
        return f"PayPal configuration for {self.user}"
