...
from apps.sales.constants import CANCELLED, NEW, PAYMENT_PENDING, WAITING, SUBSCRIPTION_DUES, SUBSCRIPTION_REFUND, \
    FAILURE, PAID, SALE, HOLDINGS, BANK, PAYOUT_MIRROR_SOURCE, PAYOUT_MIRROR_DESTINATION
from apps.sales.models import Deliverable, TransactionRecord, Invoice
from apps.sales.serializers import DeliverableValuesSerializer, SimpleTransactionSerializer, \
...
class SubscriptionReportCSV(CSVReport, ListAPIView, DateConstrained):
    ...
    def get_queryset(self):
        return TransactionRecord.objects.filter(
            category__in=[SUBSCRIPTION_DUES, SUBSCRIPTION_REFUND],
            **self.date_kwargs,
        ).exclude(status=FAILURE).order_by('created_on')
