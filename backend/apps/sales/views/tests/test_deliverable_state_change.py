...
from apps.sales.constants import STRIPE, AUTHORIZE, PAID, REFUNDED, COMPLETED, WAITING, NEW, PAYMENT_PENDING, QUEUED, \
    ...
    HOLDINGS, MONEY_HOLE_STAGE, ESCROW_HOLD, TABLE_HANDLING, CASH_DEPOSIT, FAILURE, LIMBO
...
class TestDeliverableStatusChange(APITestCase):
    ...
    def test_waitlist_deliverable_deliverable(self, _mock_notify):
        self.state_assertion('seller', 'waitlist/', initial_status=NEW, target_status=WAITING)
    ...
    @override_settings(
        TABLE_PERCENTAGE_FEE=Decimal('15'), TABLE_STATIC_FEE=Money('2.00', 'USD'),
    )
    @patch('apps.sales.tasks.withdraw_all.delay')
    def test_approve_table_deliverable(self, mock_withdraw, _mock_notify):
        ...
        record.targets.add(ref_for_instance(self.deliverable), ref_for_instance(self.deliverable.invoice))
        ...
        record2.targets.add(ref_for_instance(self.deliverable), ref_for_instance(self.deliverable.invoice))
        ...
        record3.targets.add(ref_for_instance(self.deliverable), ref_for_instance(self.deliverable.invoice))
        self.state_assertion('buyer', 'approve/', initial_status=REVIEW)
        record.refresh_from_db()
        records = TransactionRecord.objects.all()
