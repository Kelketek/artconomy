...
from apps.sales.constants import CARD, ESCROW, UNPROCESSED_EARNINGS, HOLDINGS, PENDING, PAID, OPEN, MONEY_HOLE, \
    ...
    PAYOUT_MIRROR_DESTINATION, SUBSCRIPTION_DUES, PAYMENT_PENDING, COMPLETED, TIPPING, TIP_SEND, PROCESSING_FEE, \
    THIRD_PARTY_FEE, CASH_WITHDRAW
...
from apps.sales.tests.factories import DeliverableFactory, TransactionRecordFactory, LineItemFactory, \
    StripeAccountFactory, BankAccountFactory, InvoiceFactory
from apps.sales.views.tests.fixtures.stripe_fixtures import base_charge_succeeded_event
from apps.sales.utils import finalize_deliverable, refund_deliverable, invoice_post_payment, get_term_invoice
...
class TestUnaffiliatedSales(APITestCase):
    ...
    def test_unaffiliated_sales(self):
        ...
        cash_line_item = LineItemFactory.create(
            ...
            invoice__status=PAID, amount=Money('10', 'USD'),
        )
        ...
        unknown_main_transaction = LineItemFactory.create(
            ...
            invoice__status=PAID, amount=Money('25.00', 'USD')
        )
        staff = UserFactory.create(is_superuser=True)
        self.login(staff)
...
class TestTipReport(APITestCase):
    ...
    def test_tip_report_all_data_available(self):
        ...
        invoice.save()
        transaction = TransactionRecordFactory.create(
            source=CARD, destination=HOLDINGS,
            category=TIP_SEND,
            created_on=timezone.now().replace(day=5),
            finalized_on=timezone.now().replace(day=5),
            amount=Money('20.00', 'USD'),
            remote_ids=['1234', '5678'],
            payer=invoice.bill_to, payee=user,
            status=SUCCESS,
        )
        ...
        transaction = TransactionRecordFactory.create(
            ...
            remote_ids=['1234', '5678'],
            payer=invoice.bill_to, payee=None,
            status=SUCCESS,
        )
        transaction.targets.set([ref_for_instance(invoice)])
        transaction = TransactionRecordFactory.create(
        ...
        transaction_fee.targets.set([ref_for_instance(invoice), ref_for_instance(transaction)])
        staff = UserFactory.create(is_superuser=True)
        self.login(staff)
        response = self.client.get(
    ...
    def payout_transactions(self, deliverable: Deliverable, mock_stripe, success=True, annotate_fees=True):
        ...
        if annotate_fees:
            annotate_connect_fees_for_year_month(
                year=deliverable.paid_on.year, month=deliverable.paid_on.month,
            )
