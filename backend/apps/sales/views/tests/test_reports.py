...
from apps.sales.constants import CARD, ESCROW, UNPROCESSED_EARNINGS, HOLDINGS, PENDING, PAID, OPEN, MONEY_HOLE, \
    CARD_TRANSACTION_FEES, CASH_DEPOSIT, BANK, SUCCESS, ACH_TRANSACTION_FEES, PAYOUT_MIRROR_SOURCE, \
    PAYOUT_MIRROR_DESTINATION, SUBSCRIPTION_DUES, PAYMENT_PENDING, COMPLETED
from apps.sales.models import Deliverable, TransactionRecord, StripeAccount
from apps.sales.tasks import annotate_connect_fees_for_year_month
from apps.sales.tests.factories import DeliverableFactory, TransactionRecordFactory, LineItemFactory, \
    StripeAccountFactory, BankAccountFactory
from apps.sales.views.tests.fixtures.stripe_fixtures import base_charge_succeeded_event
from apps.sales.utils import pay_deliverable, finalize_deliverable, refund_deliverable
...
class TestCustomerHoldings(APITestCase):
    def test_customer_holdings(self):
        ...
        TransactionRecordFactory.create(
            source=CARD, destination=ESCROW, amount=Money('25.00', 'USD'), payee=deliverable.order.seller,
        )
        TransactionRecordFactory.create(
            payer=None, source=UNPROCESSED_EARNINGS, destination=HOLDINGS, payee=deliverable.order.seller,
        ...
        TransactionRecordFactory.create(
            payer=deliverable.order.seller, source=CARD, destination=ESCROW, amount=Money('30.00', 'USD'), payee=buyer,
        )
...
class TestUnaffiliatedSales(APITestCase):
    ...
    def test_unaffiliated_sales(self):
        # Too Early
        LineItemFactory.create(invoice__created_on=timezone.now().replace(month=1), invoice__status=PAID)
        # Wrong Status
        LineItemFactory.create(invoice__created_on=timezone.now().replace(day=5), invoice__status=OPEN)
        ...
        card_line_item = LineItemFactory.create(
            invoice__created_on=timezone.now().replace(day=5), invoice__status=PAID,
        ...
        cash_line_item = LineItemFactory.create(
            invoice__created_on=timezone.now().replace(day=7), invoice__status=PAID, amount=Money('10', 'USD'),
        )
        ...
        unknown_main_transaction = LineItemFactory.create(
            invoice__created_on=timezone.now().replace(day=8), invoice__status=PAID, amount=Money('25.00', 'USD')
        )
        staff = UserFactory.create(is_superuser=True)
        self.login(staff)
...
class TestPayoutReport(APITestCase):
    ...
    def test_payout_report(self):
        ...
        transaction = TransactionRecordFactory.create(
            source=HOLDINGS, destination=BANK, remote_ids=['1234', '5678'],
        ...
        TransactionRecordFactory.create(source=CARD, destination=HOLDINGS, amount=Money('10.00', 'USD'))
        # Too old
        TransactionRecordFactory.create(
            source=HOLDINGS, destination=BANK, remote_ids=['1234', '5678'],
...
class TestUserPayoutReport(APITestCase):
    ...
    def test_payout_report(self):
        ...
        source_transaction = TransactionRecordFactory.create(payer=transaction.payer, payee=transaction.payee)
...
class TestSubscriptionReport(APITestCase):
    ...
    def test_subscription_report(self):
        user = UserFactory.create()
        transaction = TransactionRecordFactory.create(
            source=CARD, destination=UNPROCESSED_EARNINGS,
            category=SUBSCRIPTION_DUES,
            created_on=timezone.now().replace(day=5),
            finalized_on=timezone.now().replace(day=5),
            amount=Money('20.00', 'GBP'),
            remote_ids=['1234', '5678'],
            payer=user, payee=None,
            status=SUCCESS,
        )
        # Too old
        TransactionRecordFactory.create(
        ...
        self.assertEqual(len(lines), 1)
        self.assertEqual(lines[0]['id'], transaction.id)
        self.assertEqual(lines[0]['payer'], transaction.payer.username)
        self.assertEqual(lines[0]['remote_ids'], '1234, 5678')
        self.assertEqual(lines[0]['status'], 'Successful')
        self.assertEqual(parse(lines[0]['created_on']), transaction.created_on)
...
class TestOrderValues(APITestCase):
    ...
    def charge_transaction(self, deliverable: Deliverable, source: Literal[CARD, CASH_DEPOSIT] = CARD):
        """
        ...
        attempt = {
            ...
            'stripe_event': self.stripe_payment_event(deliverable) if source == CARD else None,
            'remote_ids': ['pi_12345'] if source == CARD else '',
        }
        success, records, message = pay_deliverable(
            attempt=attempt, deliverable=deliverable, requesting_user=self.staff,
        )
    ...
    def payout_transactions(self, deliverable: Deliverable, mock_stripe, success=True, annotate_fees=True):
        try:
        ...
        if success:
            record = TransactionRecord.objects.get(remote_ids__contains=f'outbound{deliverable.id}')
            record.status = SUCCESS
            record.save()
        if annotate_fees:
            annotate_connect_fees_for_year_month(
                year=deliverable.created_on.year, month=deliverable.created_on.month,
            )

    def test_participant_display(self):
        deliverable_normal = DeliverableFactory.create(created_on=timezone.now().replace(day=1), status=PAYMENT_PENDING)
        self.charge_transaction(deliverable_normal)
        deliverable_guest = DeliverableFactory.create(
            created_on=timezone.now().replace(day=2), status=PAYMENT_PENDING, order__buyer__guest=True,
        )
        self.charge_transaction(deliverable_guest)
        lines = self.get_lines(self.client.get('/api/sales/v1/reports/order-values/csv/'))
        ...
        self.assertEqual(lines[1]['buyer'], f'Guest #{deliverable_guest.order.buyer.id}')
        self.assertEqual(lines[1]['seller'], deliverable_guest.order.seller.username)

    def test_completed_values(self):
        deliverable = DeliverableFactory.create(
            created_on=timezone.now().replace(day=1), status=PAYMENT_PENDING, product__base_price=Money('15.00', 'USD'),
        )
        ...
        self.assertEqual(len(lines), 1)
        line = lines[0]
        self.assertEqual(line['ach_fees'], '2.29')
        self.assertEqual(line['our_fees'], '1.95')
        self.assertEqual(line['card_fees'], '0.74')
        self.assertEqual(line['profit'], '-1.08')
    ...
    def test_refunded_transaction(self):
        ...
        self.assertEqual(line['ach_fees'], '')
        self.assertEqual(line['our_fees'], '1.95')
        self.assertEqual(line['profit'], '1.21')
        self.assertEqual(line['price'], '15.00')
