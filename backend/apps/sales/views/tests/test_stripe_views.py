...
from apps.sales.constants import STRIPE, OPEN, PAYMENT_PENDING
from apps.sales.models import StripeAccount
from apps.sales import stripe as stripe_module
from apps.sales.tests.factories import StripeAccountFactory, InvoiceFactory, StripeReaderFactory, DeliverableFactory, \
    CreditCardTokenFactory
from apps.sales.views.tests.fixtures.stripe_fixtures import COUNTRIES_ARTCONOMY_OUTPUT, gen_stripe_countries_output
...
class TestStripeCountries(APITestCase):
    ...
    def test_list_countries(self, mock_stripe):
        ...
        mock_stripe.__enter__.return_value.CountrySpec.retrieve.return_value = gen_stripe_countries_output()
        ...
        result = self.client.get('/api/sales/v1/stripe-countries/')
        self.assertEqual(result.data, COUNTRIES_ARTCONOMY_OUTPUT)
        self.assertEqual(mock_stripe.__enter__.return_value.CountrySpec.retrieve.call_count, 1)
...
class TestStripeAccountLink(APITestCase):
    def test_stripe_link_creates_account(self, mock_stripe):
        ...
        mock_stripe.__enter__.return_value.AccountLink.create.return_value = {'url': 'https://stripe.com/strope/'}
...
class TestStripeAccounts(APITestCase):
    ...
    def test_stripe_account_list_one(self):
        ...
        self.assertEqual(result.data, [{'id': account.id, 'active': True, 'country': 'GB'}])
...
class TestStripePresentCard(APITestCase):
    def test_no_invoice_intent(self, _mock_stripe, _mock_model_stripe):
        ...
        response = self.client.post(f'/api/sales/v1/invoice/{invoice.id}/stripe-process-present-card/')
...
class TestStripeReaders(APITestCase):
    def test_list_readers(self, _mock_stripe):
        ...
        self.assertEqual(result.data['results'], [{'id': reader.id, 'name': reader.name}])
...
class TestSetupIntent(APITestCase):
    def test_setup_intent(self, mock_stripe):
        mock_stripe.__enter__.return_value.SetupIntent.create.return_value = {'client_secret': '1234'}
        user = UserFactory.create(stripe_token='beep')
        self.login(user)
        result = self.client.post(f'/api/sales/v1/account/{user.username}/cards/setup-intent/')
...
class TestInvoicePaymentIntent(APITestCase):
    def setUp(self) -> None:
        super().setUp()
        self.patcher = patch('apps.sales.views.helpers.create_or_update_stripe_user')
        self.patcher.start()

    def tearDown(self) -> None:
        self.patcher.stop()

    @patch('apps.sales.views.helpers.stripe')
    ...
    def test_incompatible_card(self):
        deliverable = DeliverableFactory.create(status=PAYMENT_PENDING, processor=STRIPE, invoice__status=OPEN)
        card = CreditCardTokenFactory(token='butts', stripe_token='', user=deliverable.order.buyer)
        deliverable.order.buyer.primary_card = card
        deliverable.order.buyer.save()
        self.login(deliverable.order.buyer)
        response = self.client.post(
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, {'card_id': 'That card ID is not a stripe card.'})
    ...
    def test_unexpected_state(self, mock_stripe):
        ...
        mock_api.PaymentIntent.modify.side_effect = InvalidRequestError(
            'Oops.', param='', code='payment_intent_unexpected_state',
        )
        deliverable = DeliverableFactory.create(
            status=PAYMENT_PENDING, invoice__current_intent='old_id', processor=STRIPE,
            invoice__status=OPEN,
