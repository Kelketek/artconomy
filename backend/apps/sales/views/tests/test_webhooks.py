...
from apps.sales.constants import PAID, PAYMENT_PENDING, QUEUED, IN_PROGRESS, REVIEW, CARD, ESCROW, FAILURE, SUCCESS, \
    ...
    UNPROCESSED_EARNINGS, CARD_TRANSACTION_FEES, BASE_PRICE, SUBSCRIPTION, TERM
...
class TestPayoutPaid(EnsurePlansMixin, TestCase):
    ...
    @patch('apps.sales.views.webhooks.pull_and_reconcile_payout_report')
    @patch('apps.sales.views.webhooks.stripe')
    def test_payout_paid_pull_and_reconcile(self, mock_stripe, mock_pull):
        event = base_payout_paid_event()
        event['data']['object']['id'] = 'beep'
        event['account'] = 'acct_fox'
        mock_response = Mock()
        mock_response.result = {'Stuff': 'things'}
        mock_stripe.__enter__.return_value.reporting.ReportRun.create.return_value = mock_response
        handle_stripe_event(connect=True, event=event)
        mock_pull.assert_called_with(event, mock_response)
