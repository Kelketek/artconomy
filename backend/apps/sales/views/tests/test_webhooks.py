...
from apps.lib.models import ref_for_instance, Subscription, SALE_UPDATE, Notification, TIP_RECEIVED
...
from apps.sales.constants import PAID, PAYMENT_PENDING, QUEUED, IN_PROGRESS, REVIEW, CARD, ESCROW, FAILURE, SUCCESS, \
    PENDING, NEW, AUTHORIZE, STRIPE, HOLDINGS, BANK, PAYOUT_MIRROR_SOURCE, PAYOUT_MIRROR_DESTINATION, \
    UNPROCESSED_EARNINGS, CARD_TRANSACTION_FEES, BASE_PRICE, SUBSCRIPTION, TERM, COMPLETED, TIPPING, TIP, OPEN
from apps.sales.stripe import money_to_stripe
from apps.sales.tests.factories import DeliverableFactory, WebhookRecordFactory, InvoiceFactory, \
    LineItemFactory, ServicePlanFactory, add_adjustment, RevisionFactory, CreditCardTokenFactory, \
    TransactionRecordFactory, StripeAccountFactory
...
from apps.sales.views.tests.fixtures.stripe_fixtures import base_charge_succeeded_event, \
    base_payment_method_attached_event, base_report_event, base_account_updated_event, base_payout_paid_event, \
    DUMMY_REPORT, DUMMY_REVERSE_REPORT, DUMMY_REPORT_NO_EFFECTIVE_TIME
...
class TestHandleEvent(EnsurePlansMixin, TestCase):
    ...
    def test_unknown_command(self, mock_logger):
        ...
        mock_logger.warning.assert_any_call('Unsupported event "%s" received from Stripe. Connect is %s', 'foo', False)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
@override_settings(
    STRIPE_CHARGE_PERCENTAGE=Decimal('2.90'), STRIPE_CHARGE_STATIC=Money('0.30', 'USD'),
    STRIPE_CARD_PRESENT_PERCENTAGE=Decimal('2.70'), STRIPE_CARD_PRESENT_STATIC=Money('0.05', 'USD'),
...
class TestHandleChargeEvent(EnsurePlansMixin, TransactionCheckMixin, TestCase):
    def test_deliverable_paid(self):
        ...
        deliverable.invoice.line_items.first().targets.add(ref_for_instance(SubmissionFactory.create()))
        ...
        transaction = TransactionRecord.objects.get(
            source=CARD, destination=ESCROW, payer=deliverable.order.buyer,
            payee=deliverable.order.seller, status=SUCCESS,
        )
    ...
    def test_manual_invoice(self):
        ...
        line = LineItemFactory.create(
            type=BASE_PRICE, amount=Money('15.00', 'USD'),
        ...
        total = TransactionRecord.objects.filter(source=CARD).aggregate(total=Sum('amount'))['total']
    ...
    def test_deliverable_payment_failed(self):
        ...
        transaction = TransactionRecord.objects.get(
            source=CARD, destination=ESCROW, payer=deliverable.order.buyer,
            payee=deliverable.order.seller, status=FAILURE,
        )
        deliverable.refresh_from_db()
    ...
    def test_pay_tip(self):
        ...
        user = UserFactory.create()
        deliverable = DeliverableFactory.create(
            order__buyer=user, status=COMPLETED,
        )
        self.assertEqual(Subscription.objects.filter(type=TIP_RECEIVED).count(), 1)
        line = LineItemFactory.create(
            invoice__type=TIPPING, amount=Money('10.00', 'USD'),
            type=TIP,
            invoice__current_intent=event['data']['object']['payment_intent'],
        ...
        self.assertEqual(Notification.objects.filter(event__type=TIP_RECEIVED).count(), 1)
    ...
    def test_service_extension_failed(self):
        ...
        invoice = InvoiceFactory(
            bill_to=user, current_intent=event['data']['object']['payment_intent'], type=TERM, status=OPEN,
        )
    ...
    def test_service_extension(self):
        ...
        self.assertEqual(user.service_plan_paid_through, current_date)
        CreditCardToken.objects.all().delete()
        invoice = InvoiceFactory(bill_to=user, current_intent=event['data']['object']['payment_intent'], type=TERM)
        line = LineItemFactory(invoice=invoice)
        service_plan = ServicePlanFactory()
        line.targets.add(ref_for_instance(service_plan))
        event['data']['object']['metadata'] = {
    ...
    def test_service_subscription(self):
        ...
        CreditCardToken.objects.all().delete()
        invoice = InvoiceFactory(
            bill_to=user, current_intent=event['data']['object']['payment_intent'], type=SUBSCRIPTION,
        )
        line = LineItemFactory(invoice=invoice)
        service_plan = ServicePlanFactory()
        line.targets.add(ref_for_instance(service_plan))
...
class TestPaymentAttached(EnsurePlansMixin, TestCase):
    ...
    def test_payment_attached_has_existing_primary(self):
        event = base_payment_method_attached_event()
        old_card = CreditCardTokenFactory(user__stripe_token='cus_1234', last_four='1111')
        user = old_card.user
...
class TestChargeCreated(TestCase):
    ...
    def test_capture_authorized(self, mock_stripe, mock_handle_charge_event):
        ...
        handle_stripe_event(connect=False, event=event)
        mock_stripe.__enter__.return_value.PaymentIntent.capture.assert_called_with('beep_beep')
...
class TestChargeCaptured(TestCase):
    @patch('apps.sales.views.webhooks.handle_charge_event')
    @patch('apps.sales.views.webhooks.stripe')
    def test_redirect_to_handle_charge_event(self, mock_stripe, mock_handle_charge_event):
        event = base_charge_succeeded_event()
        event['data']['object']['payment_intent'] = 'beep_beep'
        event['data']['object']['captured'] = True
        handle_stripe_event(connect=False, event=event)
...
class TestAccountUpdated(EnsurePlansMixin, TestCase):
    ...
    def test_account_updated_can_transfer(self, mock_withdraw_all):
        ...
        account = StripeAccountFactory(active=False, token=event['data']['object']['id'])
        affected_deliverable = DeliverableFactory.create(order__seller=account.user, processor=AUTHORIZE, status=NEW)
        unaffected_deliverable = DeliverableFactory.create(
            order__seller=account.user, status=QUEUED, processor=AUTHORIZE,
        )
        unrelated_deliverable = DeliverableFactory.create(status=NEW, processor=AUTHORIZE)
...
class TestPayoutPaid(EnsurePlansMixin, TestCase):
    ...
    def test_payout_paid_send_report_run(self, mock_stripe, mock_pull):
        ...
        mock_stripe.__enter__.return_value.reporting.ReportRun.create.return_value = mock_response
        handle_stripe_event(connect=True, event=event)
...
class TestReconcileReport(EnsurePlansMixin, TestCase):
    ...
    def test_sends_useful_missing_error(self, mock_requests):
        ...
        mock_requests.get.assert_called_with('https://example.com/report.csv', auth=HTTPBasicAuth('beep', ''))

    def test_updates_existing_transaction(self, mock_requests):
        event = base_report_event()
        mock_requests.get.return_value.content = DUMMY_REPORT
        existing_record = TransactionRecordFactory.create(
            remote_ids=['py_xoser87gh23o8hwer'], source=HOLDINGS, destination=BANK,
            finalized_on=None, status=PENDING,
        )
        handle_stripe_event(connect=False, event=event)
        existing_record.refresh_from_db()
        self.assertEqual(existing_record.status, SUCCESS)
        self.assertEqual(
            existing_record.finalized_on, datetime(year=2022, month=10, day=1).replace(tzinfo=dateutil.tz.UTC)
        )
        self.assertCountEqual(existing_record.remote_ids, ['py_xoser87gh23o8hwer', 'po_237uhawseivo2'])
    ...
    def test_creates_new_transaction(self, mock_requests):
        ...
        handle_stripe_event(connect=False, event=event)
        transaction = TransactionRecord.objects.get(source=PAYOUT_MIRROR_SOURCE, destination=PAYOUT_MIRROR_DESTINATION)
        self.assertEqual(transaction.amount, Money('47.86', 'GBP'))
        self.assertCountEqual(transaction.remote_ids, ['py_xoser87gh23o8hwer', 'po_237uhawseivo2'])
        self.assertCountEqual([target.target for target in transaction.targets.all()], [submission, existing_record])

    def test_creates_reversed_transaction(self, mock_requests):
        event = base_report_event()
        mock_requests.get.return_value.content = DUMMY_REVERSE_REPORT
        existing_record = TransactionRecordFactory.create(
            remote_ids=['pyr_xoser87gh23o8hwer'], source=BANK, destination=HOLDINGS,
            finalized_on=None, status=PENDING,
        )
        submission = SubmissionFactory.create(owner=existing_record.payee)
        existing_record.targets.add(ref_for_instance(submission))
        handle_stripe_event(connect=False, event=event)
        transaction = TransactionRecord.objects.get(source=PAYOUT_MIRROR_DESTINATION, destination=PAYOUT_MIRROR_SOURCE)
        self.assertEqual(transaction.amount, Money('47.86', 'GBP'))
        self.assertCountEqual(transaction.remote_ids, ['pyr_xoser87gh23o8hwer', 'po_237uhawseivo2'])
        self.assertCountEqual([target.target for target in transaction.targets.all()], [submission, existing_record])
