...
def update_invoice_statuses(apps, schema):
    ...
    Invoice.objects.filter(deliverables__escrow_disabled=False).update(record_only=False)
    ...
    Invoice.objects.filter(deliverables__status__in=[
        QUEUED, IN_PROGRESS, REVIEW, DISPUTED, COMPLETED, REFUNDED,
    ]).update(status=PAID)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(code=update_invoice_statuses)
    ]
