...
class New:
    ...
    ESCROW = 302
    HOLDINGS = 303
    # All fees put the difference for premium bonus into reserve until an order is
    # complete. When complete, these amounts are deposited into either the cash account
    # of Artconomy, or added to the user's holdings.
    RESERVE = 304
    # Earnings for which we have not yet subtracted card/bank transfer fees.
    UNPROCESSED_EARNINGS = 305
    # These two fee types will be used to keep track of fees that have been paid out to
    # card processors.
...
# TODO: account for future Dwolla fees as pending transactions, verify existing ones are
# pulled from the right place, allow for import of old card fees, and then make sure
# these outbound transactions from the old bank are accounted for:
...
def run_conversion(apps, schema):
    ...
    # First set of transactions: These are all the transactions used for orders in order
    # to pay into escrow.
    ...
    def one_time_events():
        ...
        if total < Decimal("0.00"):
            TransactionRecord.objects.create(
                ...
                created_on=opening_date,
                finalized_on=opening_date,
                note="Transferring from one bank account to another, and zeroing out "
                "the difference.",
            )
    ...
    def convert_disbursements(records):
        for record in records:
            ...
            TransactionRecord.objects.create(
                ...
                note="Finalized on date may be inaccurate due to missing data during "
                "conversion.",
            )
    ...
    def build_refund(
        ...
        if fee.amount == full_fee_amount:
            ...
            raise AssertionError(
                f"Refund accounting makes no sense! {fee.amount} + {difference} is not "
                f"{full_fee_amount}!",
            )
        TransactionRecord.objects.create(
            status=New.SUCCESS,
    ...
    def convert_escrow_holds(records):
        for record in records:
            # We've not changed this since opening, so it should work for all past
            # transactions.
            ...
            try:
                ...
                # Either the fee is equal to the full amount, or it is equal to the
                # premium fee amount. If neither is true, we've fucked up and need to
                # stop and roll back.
                ...
                if not (equal_to_premium or equal_to_full):
                    raise AssertionError(
                        f"Nonsense fee! Got {fee.amount}, expected {full_fee_amount} "
                        f"or {premium_fee_amount}!",
                    )
