...
describe("shortcuts.ts", () => {
  test("Handles a null asset", async () => {
    expect(deriveImage(null, "preview", true, Ratings.ADULT)).toBe(
  ...
  test("Handles a submission with a viewable rating", async () => {
    ...
    expect(deriveImage(submission, "blabla", false, Ratings.ADULT)).toBe(
      undefined,
    )
  })
  test("Handles a submission with an unviewable rating", async () => {
    ...
    expect(
      deriveImage(adultSubmission, "thumbnail", false, Ratings.ADULT),
    ).toBe("")
  })
