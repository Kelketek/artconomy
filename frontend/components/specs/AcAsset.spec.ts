...
describe("AcAsset.vue", () => {
  ...
  test("Loads and previews an asset", async () => {
    ...
    expect(wrapper.find(".asset-image").find("img").attributes()["src"]).toBe(
      "https://artconomy.vulpinity.com/media/art/2019/07/26/kairef-color.png",
    )
  })
  ...
  test("Uses a sensible default aspect ratio if the image cannot be displayed and none is specified.", async () => {
    const viewer = genUser()
    const submission = genSubmission()
    submission.rating = 2
    viewer.rating = 1
  ...
  test("Handles alternative file types", async () => {
    ...
    expect(wrapper.find(".asset-image").exists()).toBe(false)
    await waitFor(() =>
      expect(wrapper.find(".ac-video-player").exists()).toBe(true),
    )
  })
