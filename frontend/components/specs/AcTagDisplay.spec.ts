...
describe("AcTagDisplay.vue", () => {
  beforeEach(() => {
    store = createStore()
    mockError.mockClear()
    form = mount(Empty, vueSetup({ store })).vm.$getForm(
      "search",
      searchSchema(),
    )
  ...
  test("Loads and displays tags", async () => {
    ...
    const single = mount(Empty, vueSetup({ store })).vm.$getSingle(
      "submission",
      { endpoint: "/" },
    )
  ...
  test("Always allows staff to edit", async () => {
    setViewer({
      ...
      powers: genPowers({ moderate_content: true }),
    })
    const submission = genSubmission()
    ...
    const vm = (
      wrapper.findComponent(AcTagDisplay) as unknown as VueWrapper<
        typeof AcTagDisplay
      >
    ).vm
