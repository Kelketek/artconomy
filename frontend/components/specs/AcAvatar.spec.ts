...
describe("AcAvatar", () => {
  ...
  test("Populates via ID remotely", async () => {
    ...
    expect(mockAxios.request).toHaveBeenCalledTimes(1)
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq("/api/profiles/data/user/id/1/", "get", undefined, {}),
    )
    mockAxios.mockResponse(userResponse())
    await flushPromises()
    await wrapper.vm.$nextTick()
  ...
  test("Logs an error if it has insufficient information", async () => {
    ...
    expect(mockError).toBeCalledWith(
      "No username, no ID. We cannot load an avatar.",
    )
  })
  ...
  test("Repopulates if the username changes", async () => {
    ...
    await flushPromises()
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq("/api/profiles/account/Vulpes/", "get"),
    )
    expect(mockAxios.request).toHaveBeenCalledTimes(1)
    ...
    mockAxios.mockResponseFor(
      { url: "/api/profiles/account/Vulpes/" },
      rs(vulpes),
    )
    await flushPromises()
    await wrapper.vm.$nextTick()
    ...
    expect(wrapper.find("img").attributes().src).toBe("/static/stuff.jpg/")
  })
