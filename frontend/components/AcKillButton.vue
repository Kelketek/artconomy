<template>
  <v-btn @click="dialog = true">
    <v-icon left> {{ mdiGavel }} </v-icon>Kill submission
  </v-btn>
  <ac-form-dialog
    v-model="dialog"
    ...
    <v-row>
      <v-row cols="12" class="pa-3">
        <v-alert type="warning">
          This will kill this submission. Depending on the type of infraction,
          it may be impossible to ever upload this piece again.
        </v-alert>
      </v-row>
      <v-col cols="12">
        <ac-bound-field
          label="Reason"
          :field="killForm.fields.flag"
          field-type="v-select"
          :item-props="true"
          :items="killReasons"
        />
...
const killForm = useForm(`killForm${props.controller.name.value}`, {
  endpoint: url.value,
  fields: {
...
const killReasons = computed(() => {
  const options = Object.entries(FLAGS_SHORT).filter(
    ([value]) => parseInt(value, 10) > ReportFlags.IMPROPERLY_RATED,
  )
