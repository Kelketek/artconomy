<template>
  ...
  >
    <v-card :id="id">
      ...
      <v-toolbar v-else flat dark color="secondary" dense>
        <v-toolbar-title>{{ title }}</v-toolbar-title>
        <v-spacer />
      ...
      <v-card-text class="scrollableText" :class="{ 'pa-0': fluid }">
        <ac-form @submit.prevent="reSend">
          ...
          <ac-form-container :errors="errors" :sending="sending">
            <slot />
          </ac-form-container>
          <slot name="footer" />
          <slot name="bottom-buttons" :show-submit="showSubmit">
            <v-card-actions row wrap class="hidden-sm-and-down">
              <v-spacer />
              <v-btn variant="flat" @click="toggle = false">
                {{ cancelText }}
              </v-btn>
              <v-btn
                v-if="showSubmit"
...
const props = withDefaults(defineProps<DialogProps & AcFormDialogProps>(), {
  ...defaultDialogProps(),
  submitText: "Submit",
  cancelText: "Cancel",
  title: "",
