<template>
  <v-container class="pa-0" :fluid="fluid">
    ...
    <v-container v-else-if="controller.failed" class="pa-0">
      <slot name="failure">
        <v-row
            ...
            <slot name="failure-header">
              <p>
                <slot name="error-text">
                  Whoops! We had an issue grabbing the information that goes
                  here.
                </slot>
              </p>
            </slot>
...
const grow = computed(() =>
  Boolean((props.controller as ListController<any>).grow),
)
