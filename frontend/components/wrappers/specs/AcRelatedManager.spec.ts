...
describe("AcRelatedManager.vue", () => {
  ...
  test("Filters existing items out of results", async () => {
    ...
    await waitFor(() => expect(vm.manager).toBeTruthy())
    expect(vm.manager.filter(user1, "Fox", user1.username)).toBe(false)
    expect(vm.manager.filter(user3, "Ter", user3.username)).toBe(true)
    expect(vm.manager.filter(user2, "Bob", user2.username)).toBe(false)
    expect(vm.manager.filter(user3, "", user3.username)).toBe(false)
  })
