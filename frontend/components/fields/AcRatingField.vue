    ...
    <v-card-text>
      <v-row>
        <v-col v-if="label" cols="12">
          <label :for="(attrs.id as string) || undefined" class="v-label">{{
            label
          }}</label>
        </v-col>
          ...
          <v-row no-gutters>
            ...
            >
              <v-btn
                v-if="index <= max"
                :color="
                  String(scratch) === String(index) ? RATING_COLOR[index] : ''
                "
                :disabled="disabled"
                block
                variant="flat"
        ...
        <v-col cols="12" :class="{ disabled }">
          <v-row>
            ...
            <v-col v-if="showWarning && scratch === Ratings.EXTREME" cols="12">
              <v-alert type="warning" class="my-2">
                What has been seen cannot be unseen. By selecting this rating
                you are willingly engaging with this content.
              </v-alert>
            </v-col>
          </v-row>
        </v-col>
