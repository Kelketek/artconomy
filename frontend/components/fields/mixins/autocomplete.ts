...
export const useAutocomplete = ({
  ...
  endpoint: string
  itemValue?: string
  extraParams?: ComputedRef<RawData>
}) => {
  ...
  const unselected = computed(() => {
    if (Array.isArray(tags.value)) {
      return items.value.filter(
        (val) => (tags.value as number[]).indexOf(val[itemValue]) === -1,
      )
    } else {
      return items.value.filter(
        (val) => (tags.value as number) !== val[itemValue],
      )
    }
  })
  ...
  const itemFilter = (item: IdModel, queryText: string, itemText: string) => {
    ...
    if (props.multiple) {
      if (
        props.modelValue &&
        (props.modelValue as number[]).indexOf(item[itemValue]) !== -1
      ) {
        return false
      }
  ...
  watch(
    ...
    (newVal, oldVal) => {
      ...
      if (Array.isArray(newVal)) {
        newVal.forEach((val) => {
          if (!cachedItems.value[val] && cached[val])
            cachedItems.value[val] = cached[val]
        })
