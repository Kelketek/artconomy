...
export const useContentRatingSearch = (searchForm: FormController) => {
  const { rating, viewer } = useViewer()
  const ratingItems = computed(() => {
    return Object.keys(RATINGS_SHORT)
      .filter(
        // @ts-expect-error Not worth fixing now.
        (val: number) => val <= rating.value,
      )
      .map((key: string) => {
  ...
  const contentRatings = computed({
    get: (): RatingsValue[] => {
      return searchForm.fields.content_ratings.value
        .split(",")
        .sort()
  ...
  const showRatings = computed(() => {
    return (
      viewer.value && !viewer.value.sfw_mode && ratingItems.value.length > 1
    )
  })
  return {
    ratingItems,
