<template>
  <v-container>
    <ac-load-section v-if="isSales" :controller="stats">
      <template #default>
        <v-row v-if="stats.x" no-gutters class="text-center">
          <v-col cols="12" md="6">
            <h1>
              <router-link
            ...
            <v-row no-gutters>
              ...
              <v-col cols="6">
                {{ stats.x.active_orders }}
              </v-col>
              <v-col cols="6"> New Orders: </v-col>
              <v-col cols="6">
                {{ stats.x.new_orders }}
              </v-col>
              <v-row v-if="stats.x.escrow_enabled" class="pb-1">
                <v-col cols="12">
                  <v-card>
                    <v-card-text>
                      To see details on your current bank settings and
                      transfers, visit your
                      <a href="https://connect.stripe.com/express_login"
                        >Stripe Express Dashboard</a
                      >
                    </v-card-text>
                  </v-card>
                </v-col>
          ...
          <v-col cols="12" md="6" class="pb-2 d-flex flex-column">
            <div v-if="closed" class="pb-2">
              <strong
                >You are currently unable to take new commissions
                because:</strong
              >
              <ul>
                <li v-if="stats.x.delinquent">
                  You have an unpaid invoice. Please find it in your
                  <router-link :to="{ name: 'Invoices', params: { username } }">
                    invoice list and pay it.
                  </router-link>
                </li>
                <li v-if="stats.x.commissions_closed">
                  You have set your 'commissions closed' setting.
                </li>
                <li v-if="stats.x.load >= stats.x.max_load">
                  You have filled all of your slots. You can increase your
                  maximum slots to take on more commissions at one time in your
                  artist settings.
                </li>
                <li v-else-if="stats.x.products_available === 0">
                  You have no products available for customers to purchase. This
                  may mean there are none, they are hidden, they have reached
                  their 'Max at Once' level, or you do not have enough slots to
                  take any of your existing products on.
                </li>
              </ul>
            </div>
            <div v-else>
              <p>
                ...
                <router-link :to="{ name: 'Store', params: { username } }">
                  Manage your store here.
                </router-link>
              </p>
              <div class="py-5 d-none d-md-flex" />
            </div>
            <div class="grow" />
            <div class="align-self-center justify-end pb-2">
              <v-row>
                <v-col class="text-center d-flex">
                  <v-btn
                    color="green"
                    :to="{ name: 'InvoiceByProduct', params: { username } }"
                    variant="elevated"
    ...
    <v-tabs fixed-tabs>
      <v-tab :to="{ name: 'Current' + baseName, params: { username } }">
        Current
      </v-tab>
      <v-tab
        v-if="!isCases"
        :to="{ name: 'Waiting' + baseName, params: { username } }"
      >
        Waiting
      </v-tab>
      <v-tab :to="{ name: 'Archived' + baseName, params: { username } }">
        Archived
      </v-tab>
      <v-tab
        v-if="!isCases"
        :to="{ name: 'Cancelled' + baseName, params: { username } }"
      >
        Cancelled
      </v-tab>
    </v-tabs>
    ...
    <ac-form-dialog
      v-if="isSales"
      v-bind="broadcastForm.bind"
      v-model="showBroadcast"
      @submit.prevent="
        broadcastForm.submitThen(() => {
    ...
    >
      <v-row v-if="!confirmBroadcast">
        ...
        <v-col cols="12">
          <ac-bound-field
            ...
            :field="broadcastForm.fields.text"
      ...
      <template v-if="confirmBroadcast" #bottom-buttons>
        <v-card-actions row wrap class="hidden-sm-and-down" />
      </template>
      <template v-if="confirmBroadcast" #top-buttons>
        <v-card-actions row wrap class="hidden-sm-and-down" />
      </template>
    </ac-form-dialog>
  </v-container>
...
const stats = useSingle<CommissionStats>(
  `stats__sales__${flatten(props.username)}`,
  getSalesStatsSchema(props.username),
)
...
const closed = computed(() => {
  ...
  return (
    stats.x.commissions_closed ||
    stats.x.commissions_disabled ||
    stats.x.load >= stats.x.max_load
  )
})
// Used by tests.
