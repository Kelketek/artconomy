...
export const useProduct = <T extends SubjectiveProps & ProductProps>(props: T) => {
  ...
  const saleMode = computed({
    ...
    set: (val: boolean) => {
      if (val) {
        product.patchers.compare_at_price.model = product.patchers.base_price.model
      }
    }
  })
  ...
  watch(() => product.x?.starting_price, (newPrice?: string) => {
    ...
    if (parseFloat(product.x!.starting_price) > parseFloat(product.patchers.compare_at_price.model)) {
