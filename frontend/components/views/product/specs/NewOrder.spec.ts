...
describe("NewOrder.vue", () => {
  ...
  test("Submits a form with a registered user", async () => {
    ...
    await nextTick()
    const submitted = mockAxios.getReqByUrl(
      "/api/sales/account/Fox/products/1/order/",
    )
  ...
  test("Creates an invoice for an artist", async () => {
    ...
    await waitFor(() =>
      expect(router.currentRoute.value.name).toEqual("SaleDeliverablePayment"),
    )
    expect(router.currentRoute.value.params).toEqual({
      orderId: "1",
      deliverableId: "5",
  ...
  test("Submits a form with an unregistered user", async () => {
    ...
    await nextTick()
    await waitFor(() =>
      expect(router.currentRoute.value.query).toEqual({ stepId: "3" }),
    )
    await wrapper.find(".submit-button").trigger("click")
    await nextTick()
    const submitted = mockAxios.getReqByUrl(
  ...
  test("Fetches character info", async () => {
    ...
    await nextTick()
    const successfulRequest = mockAxios.getReqByUrl(
      "/api/profiles/data/character/id/50/",
    )
    const failedRequest = mockAxios.getReqByUrl(
      "/api/profiles/data/character/id/23/",
    )
    const character = genCharacter({ name: "Goof" })
    mockAxios.mockResponse(
      rs(genCharacter({ name: "Goof" })),
      successfulRequest,
    )
  ...
  test("Redirects to step three if the starting URL marks a higher number", async () => {
    ...
    await waitFor(() =>
      expect(router.currentRoute.value.query.stepId).toEqual("3"),
    )
  })
