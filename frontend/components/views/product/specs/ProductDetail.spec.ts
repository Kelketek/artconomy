...
function prepData() {
  ...
  const data = {
    productSingle: mount(Empty, options).vm.$getSingle("product__1", {
      endpoint: "/wat/",
    }),
    samplesList: mount(Empty, options).vm.$getList("product__1__samples", {
      endpoint: "/dude/",
    }),
    recommendedList: mount(Empty, options).vm.$getList(
      "product__1__recommendations",
      { endpoint: "/sweet/" },
    ),
  }
...
describe("ProductDetail.vue", () => {
  ...
  test("Prompts for age if the main sample is above the rating.", async () => {
    ...
    vm.product.makeReady(
      genProduct({
        primary_submission: genSubmission({ rating: Ratings.ADULT }),
      }),
    )
    await nextTick()
  ...
  test("Checks escrow availability", async () => {
    ...
    await nextTick()
    expect(vm.escrow).toBe(false)
    vm.subjectHandler.artistProfile.makeReady(
      genArtistProfile({ escrow_enabled: false }),
    )
    await nextTick()
    expect(vm.escrow).toBe(false)
  ...
  test("Handles meta content", async () => {
    ...
    expect(description).toBeTruthy()
    expect(description!.textContent).toBe(
      "[Starts at FREE] - This is a test product",
    )
  })
  ...
  test("Handles a table product", async () => {
    ...
    await waitFor(() =>
      expect(
        totalForTypes(
          getTotals(
            vm.lineItemSetMaps[0].lineItems.list.map(
