...
export const useStripeHost = ({
  clientSecret,
  readerFormUrl,
  canUpdate,
  paymentForm,
}: StripeHostArgs) => {
  const readers = useList<StripeReader>("stripeReaders", {
  ...
  watch(readerFormUrl, (newEndpoint) => {
    readerForm.endpoint = newEndpoint
  })
  const rawUpdateIntent = () => {
    clientSecret
      .post(paymentForm.rawData)
      .then(clientSecret.setX)
      .then(() => {
  ...
  const debouncedUpdateIntent = debounce(rawUpdateIntent, 250, {
    trailing: true,
  })
  const updateIntent = () => {
    if (!canUpdate.value) {
