...
describe("DeliverableOverview.vue", () => {
  ...
  const getDeliverable = (orderId: number, deliverableId: number) => {
    return {
      ...
      deliverable: empty.$getSingle(
        `order${orderId}__deliverable${deliverableId}`,
      ),
    }
  }
  afterEach(() => {
  ...
  test("Handles a null product", async () => {
    ...
    wrapper = mount(WrappedDeliverableOverview, {
      ...vueSetup({
        store,
        router,
  ...
  test("Loads with the order confirmation triggered", async () => {
    ...
    await router.push(
      "/orders/Fox/order/1/deliverables/5/overview?showConfirm=true",
    )
    wrapper = mount(WrappedDeliverableOverview, {
      ...vueSetup({
        store,
    ...
    await nextTick()
    await waitFor(() =>
      expect(wrapper.findComponent(".order-confirmation").isVisible()).toBe(
        true,
      ),
    )
  })
  ...
  `(
    ...
    async ({ mode, string }: { mode: ViewerTypeValue; string: string }) => {
      setViewer({
        store,
        user: genUser({ is_staff: true }),
        powers: genPowers({ handle_disputes: true }),
      })
      await router.push(
        `/orders/Fox/order/1/deliverables/5/overview?view_as=${string}`,
      )
      wrapper = mount(DeliverableOverview, {
        ...vueSetup({
          store,
          router,
          stubs: ["ac-revision-manager", "ac-comment-section"],
