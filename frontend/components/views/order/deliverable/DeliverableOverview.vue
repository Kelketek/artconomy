    ...
    <template v-slot:default>
      <v-row v-if="seller && deliverable.x && order.x">
        ...
        <v-col cols="12" sm="6" md="8">
          <v-toolbar dense color="black" v-if="order.x.buyer">
            ...
            <v-toolbar-title class="ml-1">
              <ac-link :to="profileLink(order.x.buyer)">{{ deriveDisplayName(order.x.buyer.username) }}</ac-link>
            ...
            <v-card-text>
              ...
              <v-row>
                <v-col class="text-center" v-if="isSeller && seller.landscape">
                  <v-btn color="green" variant="flat" class="add-deliverable"
                         @click="viewSettings.patchers.showAddDeliverable.model = true">Add Stage/Deliverable
                  </v-btn>
                </v-col>
      ...
      <v-row no-gutters>
        <v-col cols="12" v-if="$vuetify.display.mdAndUp">
          <ac-comment-section :commentList="comments" :nesting="false" :locked="!isInvolved" :guest-ok="true"
      ...
      <ac-expanded-property v-model="showConfirm">
        ...
        <v-row align="center" class="order-confirmation justify-content-center">
          ...
          <v-col cols="12" sm="6" md="9" align-self="center">
            ...
            <p>We've sent a confirmation to your email address. If you haven't received it, please check your spam
              folder.</p>
            <p>
