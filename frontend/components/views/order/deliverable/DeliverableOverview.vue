<template>
  <ac-load-section :controller="deliverable">
    <template v-slot:default v-if="seller">
              ...
              <v-row dense>
                <v-col cols="12" md="9" order="1">
                  <h2><span v-if="isSeller">Sale</span>
                    ...
                    #{{order.x.id}} <span v-if="!isSeller || !(is(NEW) || is(WAITING))">- [{{deliverable.x.name}}] Details:</span></h2>
                  <ac-patch-field :patcher="deliverable.patchers.name" label="Deliverable Name" v-if="isSeller && (is(NEW) || is(WAITING))"></ac-patch-field>
                </v-col>
                    ...
                    <v-row dense class="justify-content"  align-content="center">
                      <v-col>
                        <ac-patch-field
                          ...
                          :disabled="buyer && buyer.verified_email"
                          :refresh="false"
                        />
                        <small v-if="buyer && buyer.verified_email">Email verified. It may no longer be changed.</small>
                      </v-col>
      ...
      <ac-comment-section :commentList="comments" :nesting="false" :locked="!isInvolved || is(LIMBO) || is(MISSED)" :guest-ok="true" :show-history="isArbitrator" v-if="$vuetify.breakpoint.smAndDown" />
    </template>
  </ac-load-section>
</template>
