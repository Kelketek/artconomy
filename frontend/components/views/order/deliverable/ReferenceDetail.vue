          ...
          <template #default>
            ...
            <v-row>
              <v-col class="text-center">
                ...
                >
                  <template #default="{ on }">
                    <v-btn color="red" variant="flat" class="ml-1" v-on="on">
...
const { references, prefix, url, is, isInvolved, isArbitrator } =
  useDeliverable(props)

const reference = useSingle<Reference>(
  `${prefix.value}__reference${props.referenceId}`,
  {
...
reference
  .get()
  .then(() => markRead(reference, "sales.Reference"))
  .then(() =>
    updateLinked({
      list: references,
      key: "reference",
...
const referenceComments = useList<Comment>(
  `${prefix.value}__reference${props.referenceId}__comments`,
  {
