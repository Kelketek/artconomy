...
describe("DeliverableDetail.vue", () => {
  ...
  test("Handles a null buyer", async () => {
    ...
    wrapper = mount(DeliverableDetail, {
      ...vueSetup({
        store,
        router,
    ...
    const deliverableRequest = mockAxios.getReqByUrl(
      "/api/sales/order/1/deliverables/5/",
    )
  ...
  test("Sends a seller to their new submission", async () => {
    ...
    expect(newSubmissionRequest.url).toBe(
      "/api/sales/order/1/deliverables/5/outputs/",
    )
  ...
  test("Handles an order without a product", async () => {
    ...
    deliverableDef.status = DeliverableStatus.COMPLETED
    const orderRequest = mockAxios.getReqByUrl(
      "/api/sales/order/1/deliverables/5/",
    )
  ...
  test("Prompts to add revision to collection", async () => {
    ...
    await nextTick()
    expect(wrapper.findComponent(".add-submission-dialog").isVisible()).toBe(
      true,
    )
  })
  ...
  test("Adds character tags to submission form", async () => {
    ...
    const { deliverable, revisions, addSubmission } = getControllers({
      orderId: "1",
      deliverableId: "5",
    })
    ...
    revisions.makeReady([])
    await nextTick()
    const characterRequest = mockAxios.getReqByUrl(
      "/api/sales/order/1/deliverables/5/characters/",
    )
    ...
    await flushPromises()
    await nextTick()
    expect([...addSubmission.fields.tags.value].sort()).toEqual([
      "stuff",
      "things",
      "wat",
    ])
  ...
  test("Handles a newly created deliverable", async () => {
    ...
    const { deliverable, parentDeliverables, viewSettings } = getControllers({
      orderId: "1",
      deliverableId: "5",
    })
    deliverable.makeReady(deliverableDef)
    ...
    await waitFor(() =>
      wrapper
        .findComponent(".add-deliverable-dialog .dialog-submit")
        .trigger("click"),
    )
  ...
  test("Sets the references on a new invoice", async () => {
    ...
    const { deliverable, references, newInvoice } = getControllers({
      orderId: "1",
      deliverableId: "5",
    })
    deliverable.makeReady(genDeliverable())
