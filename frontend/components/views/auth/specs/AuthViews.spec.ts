...
describe("AuthViews.vue", () => {
  ...
  test("Retrieves and sends an order token", async () => {
    ...
    const fields = wrapper.vm.$getForm("login").fields
    expect(fields.order_claim.value).toBe(
      "0e59f96e-700f-48f0-ac13-f565846497d5",
    )
  })
  ...
  test("Syncs the value of the email field", async () => {
    ...
    await wrapper.vm.$nextTick()
    expect(otherController.fields.email.value).toBe(
      controller.fields.email.value,
    )
  })
  test("Prompts for a second attempt with 2FA", async () => {
    ...
    await submit.trigger("click")
    mockAxios.mockError!({
      response: { data: { token: ["Please provide your login token."] } },
    })
    await flushPromises()
