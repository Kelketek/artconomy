...
import {render, screen, RenderResult, fireEvent} from '@testing-library/vue'
import Social from '@/components/views/settings/Social.vue'
import {cleanUp, cloneSetup, rs, VueMountOptions, vueSetup, waitFor} from '@/specs/helpers'
...
describe('Social.vue', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = render(Social, {...cloneSetup(options), props: {username: 'Fox'}})
    prefs = genSocialSettings()
    socialPrefs = getSingle<SocialSettings>(`Fox__socialPrefs`, undefined, cloneSetup(options))
    socialLinks = getList<SocialLink>(`Fox__socialLinks`, undefined, cloneSetup(options))
    mockAxios.reset()
  })
  afterEach(() => {
    cleanUp(wrapper)
  ...
  it('Updates a social setting.', async () => {
    ...
    const promotionField = await waitFor(() => wrapper.getByLabelText('Allow Promotion'))
  ...
  it('Deletes a social link', async () => {
    ...
    await waitFor(() => expect(wrapper.queryAllByLabelText("Delete")).toEqual([]))
  })
  it('Adds a social link', async () => {
    ...
    mockAxios.mockResponse(rs({site_name: 'Facebook', identifier: 'boop', id: 5, comment: '', url: 'https://facebook.com/boop'}))
