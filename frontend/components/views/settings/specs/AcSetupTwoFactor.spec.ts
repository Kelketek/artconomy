...
describe("ac-setup-two-factor", () => {
  ...
  test("Fetches the relevant 2FA data", async () => {
    ...
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq("/api/profiles/account/Fox/auth/two-factor/totp/", "get", undefined, {
        params: {
          page: 1,
          size: 24,
        },
  ...
  test("Creates a Telegram Device", async () => {
    ...
    await waitFor(
      async () => await wrapper.find(".setup-telegram").trigger("click"),
    )
    expect(mockAxios.request).toHaveBeenCalledWith(
  ...
  test("Creates a TOTP Device", async () => {
    ...
    wrapper.vm.totpDevices.makeReady([])
    await waitFor(
      async () => await wrapper.find(".setup-totp").trigger("click"),
    )
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq("/api/profiles/account/Fox/auth/two-factor/totp/", "post", {
        name: "Phone",
      }),
    )
  })
