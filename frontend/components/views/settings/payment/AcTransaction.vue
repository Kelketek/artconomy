  ...
  <v-list-item>
    <div>
      ...
      <v-list-item-subtitle v-if="objective">
        {{ displayName(transaction.payer) }} ([{{
          ACCOUNT_TYPES[transaction.source]
        }}]) to {{ displayName(transaction.payee) }} ([{{
          ACCOUNT_TYPES[transaction.destination]
        }}])
      </v-list-item-subtitle>
      <v-list-item-subtitle v-else>
        <span v-if="outbound">to</span>
        <span v-else>from</span>&nbsp; {{ displayName(other) }}&nbsp; ({{
      ...
      <v-card>
        <v-card-text>
          <v-row no-gutters>
            <v-col cols="6">
              TXN ID:
              <ac-link :to="transactionLink" :new-tab="true">
                {{ transaction.id }}
              </ac-link>
            </v-col>
            <v-col cols="6">
            ...
            <v-col cols="6">
              <span v-if="transaction.remote_id"
                >Remote TXN ID: {{ transaction.remote_id }}</span
              >
            </v-col>
            <v-col cols="6">
              <strong>
                <span v-if="transaction.finalized_on"
                  >Finalized on
                  {{ formatDateTime(transaction.finalized_on) }}</span
                >
                <span v-else>Not Finalized</span>
              </strong>
            </v-col>
            <v-col v-if="transaction.card" cols="6">
        ...
        <v-card-actions class="text-right">
          <v-spacer />
          <v-btn color="primary" @click.stop="showDetails = false">
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
...
const isPayer = computed(() => {
  ...
  return (
    ...
    props.transaction.payer.username === props.username
  )
})
...
const isPayee = computed(() => {
  ...
  return (
    ...
    props.transaction.payee.username === props.username
  )
})
...
const outbound = computed(() => {
  return (
    isPayer.value &&
    (!isPayee.value || props.transaction.destination !== props.asAccount)
  )
})
