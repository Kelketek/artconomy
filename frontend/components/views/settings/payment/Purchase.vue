          ...
          <v-card>
            <v-card-text>
              <v-col class="text-center" cols="12">
                <p>
                  <strong
                    >Your default card will be charged for subscription
                    services.</strong
                  >
                </p>
              </v-col>
...
const AcCardManager = defineAsyncComponent(
  () => import("@/components/views/settings/payment/AcCardManager.vue"),
)
...
const ccForm = useForm(
  flatten(`${flatten(props.username)}__cards__new`),
  baseCardSchema(url.value),
)
const viewerItems = useViewer()
const viewer = viewerItems.viewer as ComputedRef<User>
const cards = useList<CreditCardToken>(
  `${flatten(props.username)}__creditCards`,
  {
    endpoint: url.value,
    paginated: false,
    socketSettings: {
