  ...
  <v-col>
    <v-row>
      <v-col v-if="noDevice" cols="12">
        <p>
          Two Factor Authentication (2FA) helps keep your account secure. If
          someone discovers your password, they still will not be able to log in
          without being able to access a device you own.
        </p>
      ...
      <v-col v-if="noDevice" cols="12">
        <v-row>
          ...
          >
            <v-card
              class="elevation-7 setup-totp"
              @click="totpDevices.postPush({ name: 'Phone' })"
            >
              <v-card-text>
                <v-row no-gutters>
...
const totpDevices = useList<TOTPDevice>("totpDevices", {
  endpoint: `${url.value}totp/`,
})
totpDevices.firstRun()

const tgDevice = useSingle<TGDevice>("tgDevice", {
  endpoint: `${url.value}tg/`,
})
tgDevice.get().catch(() => {
  tgDevice.ready = true
})
