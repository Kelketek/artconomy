...
describe("Upgrade.vue", () => {
  ...
  it("Discovers the current free plan", async () => {
    ...
    await nextTick()
    expect(
      wrapper.find("#plan-Free-column .current-plan-indicator").exists(),
    ).toBe(true)
  ...
  it("Discovers the current monthly fee plan", async () => {
    ...
    expect(
      wrapper.find("#plan-Landscape-column .manage-plan-button").exists(),
    ).toBe(true)
  })
  it("Prepares for a plan with tracking fees but no monthly charge", async () => {
    ...
    mockAxios.reset()
    await wrapper
      .find("#plan-Basic-column .select-plan-button")
      .trigger("click")
    await waitFor(() =>
      expect(
        mockAxios.getReqByUrl("/api/sales/account/Fox/cards/setup-intent/"),
      ).toBeTruthy(),
    )
    expect(
      mockAxios.getReqByUrl("/api/sales/account/Fox/premium/intent/"),
    ).toBeFalsy()
  })
  it("Prepares for a plan with a monthly charge", async () => {
    ...
    mockAxios.reset()
    await wrapper
      .find("#plan-Landscape-column .select-plan-button")
      .trigger("click")
    await waitFor(() =>
      expect(
        mockAxios.getReqByUrl("/api/sales/account/Fox/premium/intent/"),
