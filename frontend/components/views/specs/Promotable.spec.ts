...
import {cleanUp, cloneSetup, VueMountOptions, vueSetup, waitFor} from '@/specs/helpers'
...
describe('Promotable.spec.ts', () => {
  ...
  it('Loads a list of promotable users', async () => {
    ...
    const promotableUsers = getList<SocialSettings & {user: User}>('promotable_users', undefined, cloneSetup(context))
    promotableUsers.makeReady([
      {...genSocialSettings({id: 1}), user: genUser({username: 'Vulpes'})},
