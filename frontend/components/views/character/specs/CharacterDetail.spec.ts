...
describe("CharacterDetail.vue", () => {
  ...
  test("Mounts and fetches a character", async () => {
    ...
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq(
        "/api/profiles/account/Vulpes/characters/Kai/attributes/",
        "get",
        undefined,
    ...
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq(
        "/api/profiles/account/Vulpes/characters/Kai/colors/",
        "get",
        undefined,
        { signal: expect.any(Object) },
      ),
    ...
    expect(mockAxios.request).toHaveBeenCalledWith(
      rq(
        "/api/profiles/account/Vulpes/characters/Kai/share/",
        "get",
        undefined,
        { signal: expect.any(Object) },
      ),
  ...
  test("Produces a relevant link to the primary submission", async () => {
    ...
    await nextTick()
    expect(
      wrapper.find(".primary-submission-container a").attributes()["href"],
    ).toEqual("/submissions/100/")
  ...
  test("Handles a new submission when the primary is being changed", async () => {
    ...
    await wrapper
      .findComponent(AcCharacterToolbar)
      .vm.$emit("success", submission)
    expect(character.profile.patchers.primary_submission.model).toBe(
      submission.id,
    )
