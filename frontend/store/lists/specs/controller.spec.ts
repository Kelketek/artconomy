...
describe("List controller", () => {
  ...
  test("Gets the list", async () => {
    const controller = makeController()
    store.commit("lists/example/setList", [
      { id: 1, test: "thing", test2: "thingy" },
    ])
    expect(controller.list.map((x: any) => x.x)).toEqual([
      { id: 1, test: "thing", test2: "thingy" },
    ])
  })
  test("Sets the list", () => {
  ...
  test("Listens for a list", async () => {
    ...
    otherVueWrapper.vm
      .$getList("testList", { endpoint: "/" })
      .setList([{ id: 1 }, { id: 2 }, { id: 3 }])
    await vm.$nextTick()
    ...
    expect(
      wrapper.vm
        .$getList("testList")
        .list.map((item: SingleController<any>) => item.x),
  ...
  test("Fetches the next page", () => {
    ...
    controller.next().then(() => {
      ...
      expect(
        controller.list.map((x: SingleController<{ id: number }>) => x.x),
      ).toEqual([{ id: 6 }, { id: 7 }, { id: 8 }, { id: 9 }, { id: 10 }])
    })
  ...
  test("Receives new items from the server.", async () => {
    ...
    await nextTick()
    server.send({
      command: "sales.Deliverable.pk.100.line_items.LineItemSerializer.new",
      payload: { id: 5, name: "stuff" },
    })
    await nextTick()
    await flushPromises()
    ...
    expect(mockSend).toHaveBeenCalledWith("clear_watch_new", {
      app_label: "sales",
  ...
  test("Receives new items from the server, sans primary key.", async () => {
    ...
    server.send({
      command: "sales.Deliverable.line_items.LineItemSerializer.new",
      payload: { id: 5, name: "stuff" },
    })
    await nextTick()
    expect(controller.list[0].x.name).toBe("stuff")
