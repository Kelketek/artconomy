...
describe("Single controller", () => {
  ...
  test("Throws an error when trying to pull a controller from a non-existent cache", async () => {
    ...
    expect(() => {
      component.$getSingle("test")
    }).toThrow(
      Error(
        "Attempt to pull a Single which does not exist, 'test', from cache.",
      ),
    )
  })
  ...
  test("Syncs with the server", async () => {
    ...
    server.send({
      command: "boop.snoot.update.BoopSerializer.5",
