...
const mutations: MutationTree<RootFormState> = {
  ...
  setErrors(
    state: RootFormState,
    payload: { name: string; errors: FormErrorSet },
  ) {
  ...
  setFieldErrors(
    state: RootFormState,
    payload: { name: string; fields: FormError },
  ) {
    /* istanbul ignore if */
    if (state[payload.name] === undefined) {
      // Form was unloaded. No place to set errors.
  ...
  updateMeta(
    ...
    payload: {
      name: string
      meta: { [key in keyof typeof MetaToggles]: boolean }
    },
  ) {
  ...
  updateInitialData(
    state: RootFormState,
    payload: { name: string; data: RawData },
  ) {
    // Updates the data to contain whatever additional information is given.
    for (const key of Object.keys(payload.data)) {
  ...
  addField(
    ...
    payload: {
      name: string
      field: { name: string; schema: FieldSchema }
    },
  ) {
    // Adds a field to the form, with whatever data and optionally with errors pre-added.
    state[payload.name].fields[payload.field.name] = fieldFromSchema(
...
const actions: ActionTree<RootFormState, RootState> = {
  async submit({ state, commit }, payload: { name: string }) {
    ...
    return artCall({ url: form.endpoint, method: form.method, data }).then(
      (response) => {
        if (form.reset) {
