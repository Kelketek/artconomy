...
describe("Form and field controllers", () => {
  ...
  test("Counts references to forms via UID", () => {
    ...
    expect(formRegistry.uidTracking.example).toEqual([
      (wrapper.vm as any)._uid,
      (wrapper2.vm as any)._uid,
    ])
  })
  test("Adds references to controllers in the componentMap per component", () => {
    ...
    expect(formRegistry.componentMap[(wrapper.vm as any)._uid]).toEqual([
      controller1,
    ])
    expect(formRegistry.componentMap[(wrapper2.vm as any)._uid]).toEqual([
      controller1,
    ])
  })
