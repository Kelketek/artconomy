...
describe("Field validators", () => {
  ...
  `(
    "should return $result when handed the email $email.",
    async ({ email, result }) => {
      wrapper = mount(Empty, vueSetup({ store }))
      const controller = wrapper.vm.$getForm("example", {
  ...
  test.each`
    ...
    ${"test"} | ${"tess"} | ${"Fail"}    | ${["Fail"]}
  `(
    "should return $result when matching $value1 and $value2 when message is set to $error.",
    async ({ value1, value2, error, result }) => {
      wrapper = mount(Empty, vueSetup({ store }))
      const controller = wrapper.vm.$getForm("example", {
        endpoint: "",
        fields: {
          ...
          field2: {
            value: value2,
            validators: [{ name: "matches", args: ["field1", error] }],
          },
        },
      }).fields.field2
      controller.validate()
  ...
  test.each`
    ...
    ${104} | ${false}
  `(
    "should designate the success status of $status as $result.",
    async ({ status, result }) => {
      expect(validateStatus(status)).toBe(result)
    },
  )
  test("Generates an async validator that checks for a common error pattern.", async () => {
    const validator = simpleAsyncValidator(
      "/api/profiles/form-validators/email/",
    )
    wrapper = mount(Empty, vueSetup({ store }))
    const controller = wrapper.vm.$getForm("example2", {
      fields: {
        email: { value: "test@example.com", validators: [{ name: "email" }] },
      },
      endpoint: "#",
    }).fields.email
  ...
  test("Returns errors from a generated async validator.", async () => {
    ...
    mockAxios.mockResponse({
      status: 400,
      data: { email: ["Test error1", "Test error2"] },
    })
    await flushPromises()
  })
  ...
  `("Should validate a max length string", async ({ input, result }) => {
    ...
    const controller = wrapper.vm.$getForm("example", {
      fields: {
        name: { value: input, validators: [{ name: "maxLength", args: [5] }] },
      },
      endpoint: "#",
    }).fields.name
    controller.validate()
  ...
  `("Should validate a min length string", async ({ input, result }) => {
    ...
    const controller = wrapper.vm.$getForm("example", {
      fields: {
        name: { value: input, validators: [{ name: "minLength", args: [5] }] },
      },
      endpoint: "#",
    }).fields.name
    controller.validate()
  ...
  `("Validates a numeric entry", async ({ input, result }) => {
    ...
    const controller = wrapper.vm.$getForm("example", {
      fields: {
        items: { value: input, validators: [{ name: "numeric", args: [5] }] },
      },
      endpoint: "#",
    }).fields.items
    controller.validate()
