...
export const getCharacter = (
  name: string,
  schema: CharacterModuleOpts | undefined,
  context: VueMountOptions,
): CharacterController => {
  const empty = mount(Empty, context)
  return empty.vm.$getCharacter(name, schema)
}

export const getSingle = <T extends object>(
  name: string,
  schema: SingleModuleOpts<T> | undefined,
  context: VueMountOptions,
): SingleController<T> => {
  const empty = mount(Empty, context)
...
export const getList = <T extends object>(
  name: string,
  schema: ListModuleOpts | undefined,
  context: VueMountOptions,
): ListController<T> => {
  const empty = mount(Empty, context)
...
export const getForm = (
  name: string,
  schema: NamelessFormSchema | undefined,
  context: VueMountOptions,
): FormController => {
  const empty = mount(Empty, context)
