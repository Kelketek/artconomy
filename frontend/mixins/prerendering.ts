...
export const setStatus = (statusCode: HttpStatusCode) => {
  ...
  const statusHeader = document.querySelector('meta[name="prerender-status-code"]') || createStatusHeader()
  statusHeader.setAttribute('content', statusCode + '')
}

export const getHeaders = () => {
  return Object.fromEntries(Array.from(document.querySelectorAll('meta[name="prerender-header"]')).map((element: Element) => {
...
export const replaceHeaderValues = (toSet: [string, string][]) => {
  document.querySelectorAll('meta[name="prerender-header"]').forEach((element) => element.remove())
...
export const redirect = (url: string, permanent?: true) => {
  const status = permanent ? HttpStatusCode.PermanentRedirect : HttpStatusCode.TemporaryRedirect
  setStatus(status)
...
export const usePrerendering = () => {
  ...
  watch(() => window.PRERENDERING, () => prerendering.value = !!window.PRERENDERING)
  return {
