...
describe("Subjective.ts", () => {
  ...
  test("Changes the current viewer username if they are the subject.", async () => {
    ...
    await nextTick()
    expect((store.state as any).profiles.viewerRawUsername).toBe(
      "Vulpes_Veritas",
    )
  })
  ...
  test("Allows the user to see if they are not the subject and the view is private but they are a staffer with the right powers", async () => {
    await router.push("/place")
    expect((store.state as any).errors.code).toBe(0)
    const user = genUser()
    user.is_superuser = false
  ...
  test("Does not permit an unqualified staffer to access a protected view", async () => {
    expect((store.state as any).errors.code).toBe(0)
    const user = genUser()
    user.is_superuser = false
    user.is_staff = true
  ...
  test("Allows a superuser to access a protected view", async () => {
    await router.push("/place")
    expect(store.state.errors!.code).toBe(0)
    const user = genUser()
