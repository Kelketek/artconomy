...
describe("artCall", () => {
  ...
  test("Performs a GET request", () => {
    const catchFn = vi.fn()
    const thenFn = vi.fn()
    artCall({ url: "/test/location/", method: "get", data: { test: "data" } })
      .then(thenFn)
      .catch(catchFn)
...
describe("Filenname modifiers", () => {
  test.each`
    ...
    ${"test.JPG"}         | ${"JPG"}
  `(
    "should derive the extension $extension from $filename.",
    ({ filename, extension }) => {
  ...
  test.each`
    ...
    ${""}                  | ${""}
    ${"1. First"}          | ${"First"}
  `("should reduce $markdown to $text.", ({ markdown, text }) => {
...
describe("Enumerators", () => {
  ...
  test("Generates a set of rating options", () => {
    expect(ratings()).toEqual([
      ...
      {
        text: "Risque/mature, not adult content but not safe for work",
        value: "1",
      },
      { text: "Adult content, not safe for work", value: "2" },
      {
        text: "Offensive/Disturbing to most viewers, not safe for work",
        value: "3",
      },
    ])
  })
...
describe("HTTP Helpers", () => {
  ...
  test.each`
    ...
    ${"unknown"} | ${false}
  `(
    "Should determine the CSRF safety of method $method to be $result",
    ({ method, result }) => {
...
describe("Formatters", () => {
  test("Formats a datetime string", () => {
    expect(formatDateTime("2019-05-03T15:41:36.902Z")).toBe(
      "May 3rd 2019, 10:41:36 am",
    )
  })
  ...
  test("Does not truncate text that is under the limit", () => {
    expect(
      truncateText("This is a test string. It is 49 characters long.", 50),
    ).toBe("This is a test string. It is 49 characters long.")
  })
  test("Truncates text that is over the limit", () => {
    expect(
      truncateText("This is a test string. It is 49 characters long.", 4),
    ).toBe("This...")
  })
  test("Truncates before the space when over the limit", () => {
    expect(
      truncateText("This is a test string. It is 49 characters long.", 5),
    ).toBe("This...")
  })
  test("Does not truncate text mid-word if it can be avoided", () => {
    expect(
      truncateText("This is a test string. It is 49 characters long.", 12),
    ).toBe("This is a...")
  })
  test("Truncates mid-word if it has no other choice", () => {
    expect(
      truncateText("This is a test string. It is 49 characters long.", 2),
    ).toBe("Th...")
  })
  test.each`
    size              | result
  ...
  `(
    "Makes the byte size $size human readable as $result",
    ({ size, result }) => {
...
Here's an email: support@artconomy.com`),
    ).toBe(`<h1>Hello there.</h1>
...
`,
  ...
  test.each`
    ...
    ${"__deleted45"} | ${"[deleted]"}
  `(
    "Should format a display name based on a username",
    ({ userName, result }) => {
  ...
  test.each`
    ...
    ${"Fox"}      | ${false}
  `(
    "Can determine whether a username belongs to a guest account",
    ({ userName, result }) => {
...
describe("Path helpers", () => {
  test.each`
    ...
    ${null}              | ${""}
  `(
    "Should derive $result from query param set $paramSet",
    ({ paramSet, result }) => {
...
describe("makeQueryParams", () => {
  test("Wat", () => {
    ...
    expect(makeQueryParams(testData)).toEqual({
      stuff: "false",
      things: "true",
      wat: "2",
    })
...
describe("Mark Read handler", () => {
  ...
  test("Marks an item as read", async () => {
    const target = mount(Empty, vueSetup()).vm.$getSingle("boop", {
      endpoint: "/test/",
      x: { id: 1, read: false },
    })
...
describe("Linked update handler", () => {
  test("Updates a list with linked references", () => {
    const targetList = mount(Empty, vueSetup()).vm.$getList("boop", {
      endpoint: "/test/",
    })
    targetList.setList([
