...
export function newUploadSchema(
  userHandler: SingleController<TerseUser | User | AnonUser>,
) {
  return {
...
export function fallbackBoolean(
  query: LocationQueryRaw,
  field: string,
  defaultValue: any,
) {
...
export async function markRead<T extends ReadMarkable>(
...
export function updateLinked(options: LinkUpdateOptions) {
  ...
  updateItems = updateItems.filter(
    (x) =>
      x[options.key][options.subKey as string] ===
      options.newValue[options.subKey as string],
  )
  updateItems.map((x) => {
    x[options.key] = options.newValue
  })
...
export function ComputedGetters<T extends Function>(Wrapped: T): T {
  // prototype props.
...
export const prepopulateCharacters = (
  field: FieldController,
  showRef: Ref<boolean>,
  initRef: Ref<Character[]>,
) => {
    ...
    for (const charId of field.model) {
      promises.push(
        artCall({
          ...
          .then((response) => initRef.value.push(response))
          .catch(() => {
...
export const setViewer = ({
  ...
  store.registerModule(
    pathFor(username),
    new ProfileModule({ viewer: true, persistent: true }),
  )
  store.registerModule(
...
export const loadErrorHandler =
  (store: ArtStore) => (event: PromiseRejectionEvent) => {
    ...
    if (reason.name === "TypeError") {
      ...
      if (
        ...
        store.commit("pushAlert", {
          message:
            "We had an issue loading part of the page. Try clicking around a bit or refreshing.",
          category: "error",
        })
